...
class fmi3GetBinary(BaseFMI3Module):
...
"""
    ...
    def parse_struct(self, struct_content):
        # Find Binary variables and their signals
        binary_vars = []
        var_pattern = r"fmi3Binary\s+(\w+);"
    ...
    def parse_xml(self, xml_path):
        """
        Parse modelDescription.xml to extract Binary variables and their value references.
        ...
        try:
            ...
            # Find all Binary elements
            for var in root.findall(".//Binary"):
                name = var.get("name")
                value_ref = var.get("valueReference")
                if name and value_ref:
                    binary_vars[name] = int(value_ref)
